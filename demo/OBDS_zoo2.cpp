...
std::vector<int> OBDS(const torch::Tensor& imgCurr, 
    ...
    // int h = imgCurr.size(1);
    // int w = imgCurr.size(2);
    ...
    // // Start search
    // costs[4] = _costMAD(imgCurr.slice(1, y1, y2).slice(2, x1, x2), ref_block).item<float>();
    // float cost = 0.0f;
    // int point = 4;
    // if (costs[4] != 0.0f) {
    //     computations += 1;
    ...
    //             continue;
    //         }
    //         costs[k] = _costMAD(imgCurr.slice(1, yDiamond, yDiamond + blockH).slice(2, xDiamond, xDiamond + blockW), ref_block).item<float>();
    //         computations += 1;
    //     }
    ...
    //                 continue;
    //             }
    //             costs[idx] = _costMAD(imgCurr.slice(1, yDiamond, yDiamond + blockH).slice(2, xDiamond, xDiamond + blockW), ref_block).item<float>();
    //             computations += 1;
    //         }
    //     }
    ...
    // return bboxCurr;
    std::cout << "imgCurr sizes: ";
    for (const auto& s : imgCurr.sizes()) std::cout << s << " ";
    std::cout << std::endl;

    std::cout << "ref_block sizes: ";
    for (const auto& s : ref_block.sizes()) std::cout << s << " ";
    std::cout << std::endl;

    std::cout << "prev_bbox: ";
    for (const auto& v : prev_bbox) std::cout << v << " ";
    std::cout << std::endl;
    return prev_bbox;
}
