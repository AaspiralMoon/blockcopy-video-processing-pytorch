...
if __name__ == "__main__":
    ...
    for i in range(1, len(image_list)-1):
        ...
        params = [(imgCurr, ref_block, prev_bbox) for ref_block, prev_bbox in zip(ref_block_list, prev_box_list)]
        t1 = time.time()
        # new_bboxes = batch_DS_for_bbox(imgCurr, ref_block_list, prev_box_list)
        new_bboxes = list(map(lambda param: DS_for_bbox(*param), params))
        t2 = time.time()
