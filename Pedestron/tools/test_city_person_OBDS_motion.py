...
from blockcopy.policy.information_gain import get_iou
...
def gen_motion(result_motion):
    boxes0 = result_motion[0][:,:4].astype(np.int32)
    boxes1 = result_motion[1][:,:4].astype(np.int32)
    motion_vectors = []
    ...
    for box1 in boxes1:
        ...
        if matched_box is not None:
            ...
            center_matched_box = ((matched_box[0] + matched_box[2]) / 2, (matched_box[1] + matched_box[3]) / 2)
            # Calculate the motion vector and round to nearest integer
            motion_vector = (int(round(center_matched_box[0] - center_box1[0])), int(round(center_matched_box[1] - center_box1[1])))
        else:
            # No match found, return a zero vector
            motion_vector = (0, 0)
...
def apply_mv(frame, box, mv):
    ...
    if 0 <= new_x1 <= W and 0 <= new_y1 <= H and 0 <= new_x2 <= W and 0 <= new_y2 <= H:
        return np.array([new_x1, new_y1, new_x2, new_y2, score])
    else:
        return box
            ...
            for frame_id in range(clip_length):
                ...
                if frame_id == 0 or frame_id == 1:
                    ...
                    with torch.no_grad():
                        ...
                        result_motion.append(result_ref)                  
                    ...
                    motion_vectors = gen_motion(result_motion)
                    idx_del = []
                    for j, (box_prev, mv) in enumerate(zip(result_prev, motion_vectors)):
                        ref_block = frame_ref[int(result_ref[j][1]):int(result_ref[j][3]), int(result_ref[j][0]):int(result_ref[j][2])] 
                        box_prev = apply_mv(frame, box_prev, mv)
                        box_OBDS = OBDS_single(frame, ref_block, box_prev)
                        if box_OBDS is None:
                            idx_del.append(j)
                        else:
