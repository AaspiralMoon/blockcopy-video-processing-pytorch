...
def template_matching(img, template, box_prev, method='cv2.TM_CCOEFF_NORMED'):
    ...
    return np.array([top_left[0], top_left[1], bottom_right[0], bottom_right[1], score])
            ...
            for frame_id in range(clip_length):
                ...
                if frame_id == 0:
                    ...
                    for j, box_prev in enumerate(result_prev):
                        ...
                        box_OBDS = template_matching(frame, ref_block, box_prev, 'cv2.TM_CCOEFF_NORMED')
                        if box_OBDS is None:
                            idx_del.append(j)
                        else:
