...
def main():
    ...
    for i in range(args.checkpoint_start, args.checkpoint_end):
        ...
        if not distributed:
            ...
            print('# ----------- warmup ---------- #')
            outputs_warmup, _, _, _, _ = single_gpu_test(model, data_loader_warmup, False, False, '', args, limit=args.num_clips_warmup)
            ...
            outputs_eval, num_images, num_exec_list, num_est_list, num_total = single_gpu_test(model, data_loader, args.show, args.save_img, args.save_img_dir, args, limit=args.num_clips_eval)
        ...
        res_warmup = collect_res(outputs_warmup)
        res_eval = collect_res(outputs_eval)
        
        os.makedirs(os.path.dirname(args.out.replace('.json', '_warmup.json')), exist_ok=True)
        with open(args.out.replace('.json', '_warmup.json'), 'w') as f:
            json.dump(res_warmup, f)        
        os.makedirs(os.path.dirname(args.out.replace('.json', '_eval.json')), exist_ok=True)
        with open(args.out.replace('.json', '_eval.json'), 'w') as f:
            json.dump(res_eval, f)
